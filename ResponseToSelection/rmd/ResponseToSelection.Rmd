...
Z^T_yR^{-1}X_y   &  Z_y^TR^{-1}Z_y+G \otimes A_y^{-1}
\end{matrix}
\right)^{-1}
$$
...
$X_1$ is `r mfs+mhs+3` times 1.
```{r}
(mX <- rep.int(1, times=mfs+mhs+3))
```
$X_2$ is `r ffs+fhs+3` times 1.
```{r}
(fX <- rep.int(1, times=ffs+fhs+3))
```
For $Z_y$ we have the following matrix for a male candidate:

```{r}
mZ <- matrix(0,nrow=mfs+mhs+3, ncol=mfs+mhs+3)
diag(mZ)<-1
mZ[1:2,]=0
mZ
```
...
Then I compute the right bottom part which from
$$1/\sigma_e^2Z_y^TZ_y+1/\sigma_a^2A_y^{-1}$$
changed to (be aware that the whole matrix is an inverse)
$$Z_y^TZ_y+\sigma_e^2/\sigma_a^2A_y^{-1}$$
which is
```{r}
(ZZ <- crossprod(mZ)+as.vector(sige/sigg)*mAinv)
```
...
Then I can compute the transpose of X and Z with t(), which actually does not change much.
