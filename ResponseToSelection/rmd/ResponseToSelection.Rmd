...
We build a diagonal matrix with the standard deviations inside $D$ and a matrix with all the correlation but with 1,1,1,... as diagonal elements $E$.
...
matE
```
Then we get the required matrix $\Omega$ by the following equation:
$$\Omega=D \bullet E \bullet D$$
It works:
...
Now we can compute $G$ out of $\Omega$ because the pedigree of the male and female candidate is available.

To compute $G$ we need to compute the crossproduct of $\Omega$ with the relationsship matrix $A_y$ for each sex separateley:
...
Now we have $G_y$. To get $G_y^*$ and $C_y^*$ we first have to compute $C_y$. $C_y$ is the difference from $G_y$ to $PEV_y$.
$$C_y=G_y-PEV_y$$
...
For this formula you need again the relationship matrix $A_y$, but also the genetic and phenotypic variances $\sigma_g$ and $\sigma_p$.
...
For we need to compute $\sigma_e$ for each trait, it will be a vector of `r n` values. Where is $\sigma_p$?
