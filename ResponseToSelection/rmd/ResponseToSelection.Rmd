...
$R^{-1}$ is the inverse vector of variance of the environment. Each value of the vector is the difference between $\sigma_g$ and $\sigma_p$ for each trait:
...
(sigp<-sigg/h2)
(sige<-sigp-sigg)
R <- matrix(0,nrow=(mfs+mhs+3)*n,ncol=(mfs+mhs+3)*n)
diag(R)=rep(as.vector(sige),each=mfs+mhs+3)
R
Rinv <- solve(R)
Rinv
```
...
First I compute the pedigree of a male candidate. There we have 2 parents, 20 siblings and the candidate wich in total are 23 animals. The parents of the parents are unnkown.
...
```{r}
z1<-cbind(matrix(0, nrow=mhs+mfs,ncol=3),diag(mhs+mfs))
t8<-diag(n)
t8%x%z1
```
...
```{r}
mZ <- matrix(0,nrow=(mfs+mhs)*n, ncol=(mfs+mhs+3)*n)
diag(mZ)<-1
mZ[,1:3]=0
...
For $Z_y$ we have the following matrix for a female candidate:

```{r}
fZ <- matrix(0,nrow=ffs+fhs+2, ncol=ffs+fhs+3)
diag(fZ)<-1
fZ[1:2,]=0
fZ
```
