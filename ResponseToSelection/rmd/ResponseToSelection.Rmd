...
dim(G)
```
Now we have $G$, it describes how the different traits in the index are related to each other.
...
$C_1$ and $C_2$ been the last variable not yet defined in the formula
$$
\begin{bmatrix}
C_1^* \\
C_2^*
...
dim(In)
dim(Sinv%x%In)
c(mCa,fCa) <- (Sinv%x%In)%*%rbind(mC,fC) 
```
