...
$$C_y=G-PEV_y$$
You can compute $PEV_y$ and $G$ with the available data.
$G$ owns as values the covariances between each trait.
...
(matO <-matD %*% matE %*% matD)
```
Now we have $G_y$, it describes how the different traits in the index are related to each other.
The next variable we need is $PEV_y$.

However we do not have the matrix $PEV_y$ yet. 
We can compute $PEV_y$ with the formula derived from [@Phocas1996].
...
\left( 
\begin{matrix}
X^T_yR^{-1}X_y  &  X_y^TR^{-1}Z_y \\
Z^T_yR^{-1}X_y   &  Z_y^TR^{-1}Z_y+A_y^{-1} \otimes G^{-1}
\end{matrix}
\right)^{-1}
$$

For this formula you need the relationship matrix $A_y$, but also the genetic and phenotypic variances $\sigma_g$ and $\sigma_p$ and $G$.
You will not need any other data.

$Z$ is a design matrix that connects the observations with the breeding values.
$X$ is a vector of the values 1,1,1, as long as there are animals involved. It could connect the observations to fixed factors, but we do not have any.
$R^{-1}$ is the inverse diagonal matrix of the values of variance of the environment for the traits and the recorded animals. 
Each diagonal value of the vector is the difference between $\sigma_g$ and $\sigma_p$ for each trait:
$$\sigma_e=\sigma_p-\sigma_g$$
For we need to compute $\sigma_e$ for each trait, it will be a vector of `r n` values.

$\sigma_g$ is available and $\sigma_p$ is possible to compute from the heritability $h^2$ for each trait:
...
Therefore the candidate and the parents should not own an element in the vector, but all other animals that have a record own an element of each trait. 
This results in a diagonal matrix $R_1$ with a `r n*(mfs+mhs)` x `r n*(mfs+mhs)` dimension for a male candidate.
...
(sigp<-sigg/h2)
(sige<-sigp-sigg)
R <- matrix(0,nrow=(mfs+mhs)*n,ncol=(mfs+mhs)*n)
diag(R)=rep(as.vector(sige),each=mfs+mhs)
R
mRinv <- solve(R)
```
Now we have $R^{-1}_1$.
...
Here I have to compute $R^{-1}_2$ aswell.




The next variable we need is the inverse of the relationship matrix $A_y$ for each sex.

We know that the male candidate has `r mfs` full siblings, `r mhs` half siblings and a female has `r ffs` full siblings and `r fhs` half siblings.

Based on the number of siblings we can create a relationship matrix.
We can compute the genetic additive relationship matrix $A$ for a given pedigree using the `pedigreemm` package.
We just need to fill in the information about the parents of each animal.

First I compute the pedigree of a male candidate. There are 2 parents, 20 siblings and the candidate wich in total are 23 animals. The parents of the parents are unnkown.
...
Then I need to do the same for a female candidate:
A female candidate has `r ffs` full siblings and `r fhs` half siblings. And 2 parents. And the candidate. Which is a total of 14 animals.
```{r}
suppressPackageStartupMessages( library(pedigreemm))
fnumb <- ffs+fhs+3
...
```
Then I create a matrix out of the pedigree:
The function `getA` computes the additive relationship matrix $A$.

```{r}
getA(fped)
```

The inverse $A^{-1}$ is computed as 
...
Only the vector $X_y$ and the design matrix $Z_y$ are still missing.

$X_y$ is a matrix that connects the records with the levels of fixed effects.
I do not have any fixed effects.
Therefore we can assume that $X_y$ is just a vector with the number of values that correspond to the number of records.
...
Then I compute $X_2$.
$X_2$ is `r ffs+fhs+3` times 1.
```{r}
(fX <- rep.int(1, times=(ffs+fhs)*n))
```


For $Z_1$ I can compute the required matrix by a kronecker product.
I have the matrix $z1$ that tells us what $Z_1$ would be for only one trait and I have the unit matrix $t8$ that has the size of the number of traits.
...
For $Z_2$ we have the following matrix for a female candidate:

```{r}
fZ<-diag(n)%x%cbind(matrix(0, nrow=fhs+ffs,ncol=3),diag(fhs+ffs))
dim(fZ)
```


I have to compute the inverse of $G$ and multiply each value

I only have to compute the inverse of $G$, then I can get started.
...
Now I can compute the matrix $PEV_1$ first for a male candidate.
To compute it I have to compute first all four parts of the whole matrix. 


First I will compute the left top part
$$1/\sigma_e^2X_y^TX_y$$
Which is



Now I can compute the matrix $PEV_y$ first for a male candidate.
To compute it I have to compute first all four parts of the whole matrix. 
Also we can take out the $\sigma_e$ of the matrix elements and multiply the whole matrix afterwards with it. 
Then I need to change the parts of the matrix $PEV_y$ aswell:
...
Then I compute the right top part which from
$$1/\sigma_e^2X_y^TZ_y$$
becomes
$$X_y^TZ_y$$
which is
...
Then I compute the left bottom part which from
$$1/\sigma_e^2Z^T_yX_y$$
Changed to
$$Z^T_yX_y$$
Which is
...
(mat_coeff <- rbind(cbind(XtX, XtZ), cbind(t(XtZ), ZtZ)))
```
...
We can further define the vector of the economical weights $a$ which is the change in revenue [Sfr.] per genetic standard deviation.
