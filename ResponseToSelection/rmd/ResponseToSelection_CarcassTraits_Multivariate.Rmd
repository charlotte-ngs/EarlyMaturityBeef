...
setwd("~/Documents/EarlyMaturityBeef/ResponseToSelection/rmd")
## male
male_proportionselected <- "AN_proportion_selected"
...
## female
female_proportionselected <- "AN_proportion_selected"
...
### Asymptotic genetic gain of a trait with aggregate breeding value involving all traits (here carcass fat)
...
QcCF <- (male_i*as.numeric(t(a)%*%male_Ca[,"cCF"])*sqrt(as.numeric(t(a)%*%male_Ca%*%a))+female_i*as.numeric(t(a)%*%female_Ca[,"cCF"])*sqrt(as.numeric(t(a)%*%female_Ca%*%a)))/(male_generationintervall+female_generationintervall)
...
index <- data.frame(trait=c("CFa","CFc","CCa","CCc","CWa","CWc"), Sfr=c(QaCF,QcCF,QaCC,QcCC,QaCW,QcCW))
sum(c(QaCF,QcCF,QaCC,QcCC,QaCW,QcCW))
```

### Asymptotic genetic gain of a trait in the aggregate breeding value involving not all traits

$$b=\sigma_n^{-1}(G_n)^Ta$$
Where $n$ indicates the trait. Here aCF.
```{r}
CFaB <- (1/G[which( colnames(G)=="aCF" ),"aCF"])*(G[,"aCF"]*a)
CFcB <- (1/G[which( colnames(G)=="cCF" ),"aCF"])*(G[,"cCF"]*a)
CCaB <- (1/G[which( colnames(G)=="aCC" ),"aCF"])*(G[,"aCC"]*a)
CCcB <- (1/G[which( colnames(G)=="cCC" ),"aCF"])*(G[,"cCC"]*a)
CWaB <- (1/G[which( colnames(G)=="aCW" ),"aCF"])*(G[,"aCW"]*a)
CWcB <- (1/G[which( colnames(G)=="cCW" ),"aCF"])*(G[,"cCW"]*a)
...
```{r GeneticGainTrait}
CFaQ <- (male_i*as.numeric(t(CFaB)%*%male_Ca[,"aCF"])*sqrt(as.numeric(t(CFaB)%*%male_Ca%*%CFaB))+female_i*as.numeric(t(CFaB)%*%female_Ca[,"aCF"])*sqrt(as.numeric(t(CFaB)%*%female_Ca%*%CFaB)))/(male_generationintervall+female_generationintervall)

CFcQ <- (male_i*as.numeric(t(CFcB)%*%male_Ca[,"cCF"])*sqrt(as.numeric(t(CFcB)%*%male_Ca%*%CFcB))+female_i*as.numeric(t(CFcB)%*%female_Ca[,"cCF"])*sqrt(as.numeric(t(CFcB)%*%female_Ca%*%CFcB)))/(male_generationintervall+female_generationintervall)
...
```{r}
library(ggplot2)
dataframe <- data.frame(names = rep(c("CFa","CFc","CCa","CCc","CWa","CWc"),times=2), Sfr = c(CFaQ,CFcQ,CCaQ,CCcQ,CWaQ,CWcQ,QaCF,QcCF,QaCC,QcCC,QaCW,QcCW), Strategy=rep(rep(c("CFa","index"),each=6),times=2))
