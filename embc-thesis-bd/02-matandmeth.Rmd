...
Two tools were used to predict the effects of the two strategies: The genetic gain (see Section  \@ref(tab:carcasscategory)) and the rank correlations (see Section \@ref(tab:rankcorr)).
...
```{r angusgraphic ,fig.cap = "Angus beef animal (Retrieved on 3.1.2019 from https://www.mutterkuh.ch/de/herdebuch/rasse/angus)."}
knitr::include_graphics(system.file("extdata","Angus.jpg", package="Exemplary"))
```

```{r limousingraphic ,fig.cap = "Limousin beef animal (Retrieved on 3.1.2019 from https://www.mutterkuh.ch/de/herdebuch/rasse/limousin)."}
...
Both vectors had one entry for each trait which took the same place in $a$ and $û$ (length of $a$ and $û$ = 6).
...
The residual and genetic variances / covariances were computed by the method "estimation of variance components" using the software VCE created by Groeneveld.
They relied on data of estimated breeding values $û$ which were computed using a multivariate best linear unbiased prediction (BLUP) model [@Kunz2018].
...
where $a$ was the vector of economic weights and $û_{Index}$ was the vector of estimated breeding values of all six carcass traits (length of $a$ and $û_{Index}$ = 6).
...
where $Z$ was the design matrix, $R$ was the expanded residual variance / covariance matrix (see Table \@ref(tab:resvarcov)) and $[1:n,1:n]$ signaled that only the first $n$ rows and columns were used as $PEV$ (belonged to the selection candidate).
...
* breeding value for carcass fat of *adults* or
