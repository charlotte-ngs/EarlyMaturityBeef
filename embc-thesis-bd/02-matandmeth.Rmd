...
g + geom_bar(stat="identity", width=0.5, fill="tomato2") +
  ...
  theme_classic()+
  ylab("Population frequency")+
  xlab("Carcass fat class")+theme(text = element_text(size = 10))
```
...
From its value correlated genetic gains $Q_m^*$ were computed for all carcass traits individually.
They express the mean change in trait unit or genetic standard deviation per year.
Equation correlated genetic gain???
...
$$Cov(I,H)^*=\sqrt{b'P^*b}$$
where $P^*$ was the asymptotic variance covariance matrix between the information sources of $I$ and $b$ was the vector of regression coefficients to maximize the correlation between $I$ and $H$.
...
This resulted in the covariance between selection index and aggregate genotype

$$Cov(I,H)^*_{CF}=\sqrt{b'P_{CF}^*b}$$
where $b$ was  $P_{CF}$ was the variance covariance matrix $P$ between $û_{CF}$.
...
There $b$ could be replaced by the vector of economic weights $a$ because the variance covariance matrix $P$ between the information sources in $I$ was the same as the covariance matrix $G$ between $I$ and $H$ in the definition of $b$
\begin{equation}
\label{regrcoeff}
b=P^{-1}Ga
\end{equation}
...
This resulted in the two different equations
The asymptotic accuracy of the index $r^*$ was defined as the covariance between $I$ and $H$
