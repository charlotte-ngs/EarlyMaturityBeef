...
The more incissors the physiologically older the animal. Table \@ref(tab:slaughtercategory) contains the list of carcass categories and their description used in CHTAX.

The choice of slaughter category determines the basis price of an animal per kilogram carcass weight (CW).
Slaughterhouses have to determine the carcass weight by weighing the beforehand trimmed carcass. Head, tail, guts and surface fat and lower legs are not included in the carcass weight [@EDI2013, S.1].
...
The second step in the classification system CHTAX is to assign the carcass to a carcass conformation class and a carcass fat class within its slaughter category.
...
The assigned carcass conformation class depends on the accentuation of certain muscles and on the profile the animals body shows.
...
The determination of carcass fat class can further increase or decrease the base price per kilogramm carcass weight.
...
x<-data.frame(class = c("1", "2", "3", "4", "5"), coverage=c("uncovered", "partially covered", "evenly covered", "strongly covered", "excessiveley fatty"), description=c("no fat coverage","light fat coverage; muscels partially visible", "average even fat coverage; muscles generally covered", "strong fat coverage; excessive fat deposition", "fat coverage generally excessive; bulging fat formations"))
kable(x, booktabs=T, caption="Description and fat coverage of carcass fat classes") %>%
kable_styling(full_width =T) %>%
column_spec(3, width="8cm")
...
kable(x, booktabs=T, caption="Description and profile of carcass conformation classes") %>%
kable_styling(full_width =T) %>%
column_spec(3, width="8cm")
```
...
Carcass fat should be highly correlated with early maturity
...
Multivariate BLUP is a statistical model combining the given knowledge about animal pedigrees, trait heritabilities, trait variancec, trait covariances, environment factors and individual phenotypic records.
Multivariate means that BLUP accounts for the phenotypic, genetic and residual covariances between traits. 
...
* carcass fat as described in \@ref(tab:fatclass) 
* carcass conformation as described in \@ref(tab:conformationclass)
* carcass weight [dt]

Proviande employees have recorded the phenotypical observations of these traits at slaughterhouses in Switzerland during classification CHTAX of beef carcasses as described in section \ref(CHTAX). 
Each beef carcass delivers one 
The phenotypic observations have then been divided into two groups based on age at slaughter of the animal which delivered the phenotypic observation.

While the group "calves (c)" included animals which have been less or 180 d old at slaughter, the group "stirks (s)" included animals which have been more than 180 d but less than 700 d old at slaughter.
Animals 700 d or older at slaughter have not been included in the model.

The two groups c and s formed separate breeding values of the carcass traits.
...
The results shown as genetic and phenotypic variances/covariances are available at \@ref(tab:variancecomponents).

```{r variancecomponents, echo=FALSE}
library(knitr)
library(kableExtra)
x<-data.frame(c("cCC","cCF", "cCW"), c(0.4015, -0.00993, 0.01828), c( -0.00993, 0.12067,0.00050), c(0.01828,0.00050,0.00311))
colnames(x) <- c(" ","cCC","cCF", "cCW")

z<-y<-data.frame(c(0.35845, 0.13154, 0.02907), c( 0.35845, 0.23651,0.02463), c(0.02907,0.23651,0.00904))
colnames(z) <- c("cCC","cCF", "cCW")

y<-data.frame(c(0.79947, 0.14951, 0.05394), c( 0.14951, 0.39365,0.02979), c(0.05394,0.02979,0.06395), c(0.50221, 0.30654, 0.22078))
colnames(y) <- c("cCC","cCF", "cCW", "heritability")

x<-cbind(x,z,y)
kable(x, booktabs=T, caption="Genetic and phenotypic variances-covariances for calves (c) derived from Kunz (2018)") %>%
...
library(knitr)
library(kableExtra)
x<-data.frame(c("sCC","sCF", "sCW"), c(0.4013, -0.00655, 0.03387), c( -0.00655, 0.13028,0.01322), c(0.03387,0.01322,0.01947))
colnames(x) <- c(" ","sCC","sCF", "sCW")
y<-data.frame(c(0.71754, 0.07159, 0.09911), c(0.07159, 0.36439,0.04989), c(0.09911,0.04989,0.06395), c(0.55928, 0.35752, 0.30447))
colnames(y) <- c("sCC","sCF", "sCW", "heritability")
x<-cbind(x,y)
kable(x, booktabs=T, caption="Genetic and phenotypic variances-covariances for stirks (s) derived from Kunz (2018)") %>%
kable_styling(latex_options=c("hold_position")) %>%
 add_header_above(c("","genetic"=3,"phenotypic"=3,""))
