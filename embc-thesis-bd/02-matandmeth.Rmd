...
where $Cov(u_m,I)^*$ was the asymptotic covariance between a true breeding value of trait $m$ and the aggregate genotype $H$ and $Var(I)^*$ was
...
\end{equation}
$$Cov(I,H)^*=\sqrt{b'P^*b}$$
where $P^*$ was the asymptotic variance covariance matrix between the information sources of the selection index $I$ and $b$ was the vector of regression coefficients to maximize the correlation between $I$ and the aggregate genotype $H$.
The vector of regression coefficients was computed as
\begin{equation}
\label{regrcoeff}
b=P^{-1}Ga
\end{equation}
