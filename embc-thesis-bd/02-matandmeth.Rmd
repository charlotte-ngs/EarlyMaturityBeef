...
To compute the breeding values the best linear unbiased prediction (BLUP) model has been used.
BLUP is a statistical model assuming that the breeding value of an animal is the genetic additive effect on the phenotypic observation of a measurable trait of an animal.
The phenotypic observation in this case is the class of carcass fat as described in \@ref(tab:fatclass) the animal belongs to.
The genetic additive properties of an animal are not the only effect on its phenotypical observation.
A phenotypical observation of an animal is also dependent on the environmental conditions the animal lives in and on the genetic effects of dominance and epistasis.
...
The known fixed effects are included in BlUP and are subtracted from the phenotypical observation to get the breeding value.
The unnkown random effects are 
