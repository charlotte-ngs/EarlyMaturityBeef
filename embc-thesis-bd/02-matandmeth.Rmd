...
1. Carcass fat as auxilliary trait (CFAT)
...
Predictions assumed within-breed-breeding and were performed for the Swiss breed populations of Angus (Referenz für Bild Angus und Beschreibung: Angus is a extensive beef breed originating from the UK??) and Limousin (Referenz für Bild: Limousin is an intensive beef breed originating from France??).
...
It was computed separateley for each breed Angus and Limousin.
...
Both vectors had one entry for each trait which took the same place in $a$ and $û$.
...
\end{equation}
$$G^*_m=C^*_{m,Index}$$
where $C^*_{m,CF}$ was the covariance matrix between true and estimated breeding values of a trait $m$ and the two estimated breeding values in $û_{Index}$ (resulted in a vector of length 6).
...
where $k$ was the factor of variance reduction, $n$ was the number of traits involved and $I$ was the identity matrix of dimension $n$, which is the number of all carcass traits.
...
To get the covariance matrix between true and estimated breeding value before selection $C_y$ the equation
...
where $A$ was the numerator relationship matrix of dimension q x q (q = number of offspring), $\otimes$ symbolized the kronecker multiplication, $V$ was the genetic variance/covariance matrix (see Table ??) and $PEV$ was the prediction error variance matrix was used.
...
\end{equation}
$$PEV_y=(Z_y'R_y^{-1}Z_y+A_y^{-1} \otimes G^{-1})^{-1}[1:n,1:n]$$
where $Z$ was the design matrix, $R$ was the residual variance/covariance matrix (see Table ???) and $[1:n,1:n]$ signaled that only the first $n$ rows and columns (belonged to the selection candidate) were used as $PEV$.
The numerator relationship matrix $A$ was computed using the R package `pedigreem`.

## Rank correlations

Rank correlations are used to determine how strongly different selection criteria influence the ranks of animals in a population.

### Material

To compute the rank correlations, the breeding values of a set of each Angus and Limousin Swiss animals were used.
The set of Angus consisted in 83 male animals and the set of Limousin consisted in 354 male animals.
All these animals had at least 20 daughters and possessed breeding values for all six breeding values.

### Methods

Rank correlations were computed with the R command `cor(method ="spearman")`.

To get the ranks, the animals were assigned to a rank according to the values:

* breeding value for carcass fat of *adults*,

* breeding value for carcass fat of *calves* and

* the weighed sum of all carcass traits the economic weights present in Table ??.

Ranking was performed using the R command `rank(ties.method = "max")`.
