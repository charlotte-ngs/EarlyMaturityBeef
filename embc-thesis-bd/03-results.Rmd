...
The economic weights in Sfr. per trait unit are shown in Table \@ref(tab:ecweightstab).
Additionally these weights are shown corrected for the trait's genetic standard deviation in the Swiss beef population (see Figure \@ref(fig:ecweightsgensdfig)).
The correction of the economic weights allows to compare their values not only between breeds but also between traits.
```{r ecweightstab}
library(knitr)
...
g + geom_bar(stat="identity", width=0.5,aes(fill=Breed),position="dodge")+
  ...
  xlab("Carcass trait")
```
There it can be seen that carcass conformation was the most important trait for both animal groups and both breeds, followed by carcass fat.
The economic weights for carcass weight showed generally the lowest absolute values and were all negative.
The negative values for carcass weight are due to the payment system.
...
Their higher importance were due to their higher relative price differences for the carcass traits. 
Between the breeds Angus and Limousin there existed some general differences in their economic weights.
They behaved the same for both animal groups *adults* and *calves*.
Firstly, the economic weights for carcass weight were further in the negative for Limousin than for Angus.
This was due to higher population means in carcass weight for Limousin than for Angus (see Table \@ref(tab:cwmeansd)).
Secondly, the economic weights for carcass conformation were higher for Angus than for Limousin.
This was due to higher population means in carcass conformation for Limousin than for Angus (see Figure \@ref(fig:ccfrequencies)).
Thirdly, the economic weights for carcass fat were higher for Limousin than for Angus.
This was due to population means closer to class 3 in carcass fat for Angus than for Limousin (see Figure \@ref(fig:cffrequencies)).
Additionally economic weight was negative for carcass fat in the animal group adults and the breed Angus.
This was due to the population mean of adults in carcass fat for Angus, which was slightly above the optimum carcass fat class 3.

## Genetic Gain

Both breeding strategies led to positive genetic gains (see Figure \@ref(fig:gengainagggenotypefig)).
This shows that breeding for them would result in generally higher prices per kg carcass weight for beef carcasses of the breed populations Angus and Limousin in Switzerland.
...
This was due to more available information for the breeders to breed for the aggregate genotype when using the strategy *Index over carcass traits*.
It can be seen in the two definitions of the selection indices which are shown as Equation \@ref(selindindex) and Equation \@ref(selindcf)).
The difference between the two strategies in genetic gain was lower for Limousin than for Angus.
This was due to a lower population mean in carcass fat for Limousin than for Angus (see Figure \@ref(fig:ccfrequencies)).
...
Strategy *Carcass fat as auxilliary trait* considerably increased the population mean in this trait, which was more favorable for Limousin than for Angus (see Figure \@ref(fig:corrgengaincffig)).
The correlated genetic gains of this strategy did only differ slightly between breeds.
There the breed-specific economic weights apparently had little impact on the correlated genetic gains.
The correlated genetic gains for carcass conformation and carcass weight were positive in the group *adults* but negative in the group *calves*.
These gains were on low absolute levels in comparison to the gains in carcass fat.
This was due to positive genetic correlations between carcass fat and the *adults* carcass conformation and carcass weight, and negative genetic correlations between carcass fat and the *calves* carcass conformation and carcass weight (see Table \@ref(tab:genvarcov)).
...
```{r corrgengaincffig, fig.cap="Correlated genetic gains per genetic standard deviation of Angus and Limousin when selecting for the strategy Carcass fat as auxilliary trait."}
...
g + geom_bar(stat="identity", width=0.5,aes(fill=Breed),position="dodge")+
  theme_classic()+theme(text = element_text(size = 15))+
  ylab("Genetic gain in trait unit/y")+facet_grid(.~Group)+
  xlab("Carcass trait")
```

```{r corrgengainindexfig, fig.cap="Correlated genetic gains per genetic standard deviation of Angus and Limousin when selecting for the strategy Index over carcass traits."}
# Per genetic standard deviation
library(ggplot2)
library(varhandle)
...
Strategy *Index over carcass traits* led to negative genetic gains in carcass weight (see Figure \@ref(fig:corrgengainindexfig)).
This was due to generally low correlations between carcass weight and the other traits and represent the negative economic weights for these traits (see Table \@ref(tab:ecweightstab)).
The correlated gains for this strategy differed substantially between Angus and Limousin.
Firstly, the correlated genetic gain of carcass weight was further negative for Limousin than for Angus.
This was due to the further negative economic weights of carcass weight for Limousin than for Angus and the positive genetic correlation between carcass weight *calves* and carcass weight *adults*.
Secondly, the gain of carcass conformation was higher for Angus than for Limousin, the inverse applies to the gain of carcass fat.
These values followed the respective economic weights of these traits in the aggregate genotype.
Additionally there were two observations which could not be explained by the economic weights.
...
The ranks of the 83 Angus animals and the 354 Limousin animals were computed using their breeding values of carcass fat adults, carcass fat calves and using their index of carcass traits value.
...
The correlation coefficients are higher between the ranks of carcass fat calves and the ranks of the index, than between carcass fat adults and the ranks of the index.
This is due to the higher economic weights in the index of carcass fat calves than of carcass fat adults.
Limousin shows higher rank correlations between both carcass fat traits and the index than Angus.
The rank correlations between carcass fat and index of the Angus animals are close to 0.
This is due to the higher economic weights in the index for carcass fat of Limousin than of Angus.
