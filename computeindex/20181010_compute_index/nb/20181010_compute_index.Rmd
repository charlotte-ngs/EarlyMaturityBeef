...
pn_delta_mean <- readr::read_csv2(file = delta_mean_fn)%>%filter(Trait == "cfc")%>%select(Delta)
# Define price vector
price_fname <- "cfc_price.csv"
pvec_price <- readr::read_table(price_fname)
# Define frequency vector
freq_fname <- "lm_cfc_freq.csv"
pvec_class_freq <- readr::read_table(freq_fname)
# Define threshold vector
thresh_fname <- "cwa_threshold.csv"
pvec_threshold <- readr::read_table(thresh_fname)

# Compute economic value
compute_economic_value(pvec_class_freq = pvec_class_freq, pn_sd = pn_sd, pn_mean = pn_mean, pvec_price = pvec_price, pn_delta_mean = pn_delta_mean)
