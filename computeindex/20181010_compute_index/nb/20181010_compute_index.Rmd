  ...
  if(is.null(pvec_class_freq)) {
    ...
    # computation of economic value for carcass weight
      if(length(pvec_threshold)!=length(pvec_price)-1||length(pn_mean)>1||length(pn_sd)>1||length(pn_delta_mean)>1||pn_sd<0){
        paste("input variables do not fit to function")
        return()
        }
...
compute_economic_value(pvec_class_freq = c(2,2,35,278,634,708,298), pn_sd = 0.99, pn_mean = 5.48, pvec_price = c(11.2,12.7,13.6,14.2,14.7,15.2,15.7), pn_delta_mean = 0.01)
