...
# Define mean
s_mean_sd_fn <- "mean_sd.csv"
pn_mean <- readr::read_csv2(file = s_mean_sd_fn)%>% filter(Trait == "cfc") %>% filter(Breed == "an") %>% select(Mean)
pn_mean
# Define standard deviation
s_mean_sd_fn <- "mean_sd.csv"
pn_sd <- readr::read_csv2(file = s_mean_sd_fn) %>% filter(Trait == "cfc") %>% filter(Breed == "an") %>% select(Sd)
pn_sd
# Define delta mean
delta_mean_fn <- "delta_mean.csv"
pn_delta_mean <- readr::read_csv2(file = delta_mean_fn)%>%filter(Trait == "cf")%>%select(Delta)
