...
* Why do the references for table not work for R markdown?
...
* Should I include mean and standard deviation in the legend, how?
    *
    ...
    (ref:foo) Defined by @Agristat2018
...
* How can I use a path to my files which always works in my thesis?
    * I have to use the command system.file("extdata","library.bib",package="Exemplary") where library.bib is the file I need.
    
```{r}
knitr::include_graphics("`r system.file('extdata','Qualitas',package='Exemplary')`")
```
...
watch \@ref(tab:calfgenticvariance-covariancematrix)

```{r calfgenticvariance-covariancematrix}
library(knitr)
library(kableExtra)
...
```{r genetictrendCF, echo=FALSE, fig.cap="yey", out.width="40%"}
...
```{r}
sg<-data.frame(p=c("sCC","sCF", "sCW"), sCC=c(0.4013, -0.00655, 0.03387), sCF=c( -0.00655, 0.13028,0.01322), sCW=c(0.03387,0.01322,0.01947))
