...
* __Heritability__ ($h^2$) Selection success (mean of progeny) divided by selection differential (mean of selected parents). The higher, the more the trait is dependent on the genetics of the population [@VonRohr2017].
...
* __Response to selection__ (R) $R=ih^2\sigma_{P}$
...
The absolute value of any EBV is not critical, but rather the differences in EBVs between animals.  Particular animals should be viewed as being "above or below breed average" for a particular trait.   http://abri.une.edu.au/online/pages/understanding_ebvs_char.htm. an estimate of the ability of an individual to produce superior offspring; based on one or more measurements of performance, using phenotypic values, taken on the animal itself or, more commonly, on a number of its relatives https://medical-dictionary.thefreedictionary.com/true+breeding+value.
...
* __Selection accuracy__ [%] is based on the amount of performance information available on the animal and its close relatives - particularly the number of progeny analysed.  Accuracy is also based on the heritability of the trait and the genetic correlations with other recorded traits.  Hence accuracy indicates the "confidence level" of the EBV.  The higher the accuracy value the lower the likelihood of change in the animal's EBV as more information is analysed for that animal or its relatives [@ABRI2018]. 

* __Mid-parent regression__ 

* __Additive genetic variance__ Square of the standard deviation of breeding values.
